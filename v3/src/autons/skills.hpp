...
void corner1() {
    ...
    chass.turn(neg(14), {.timeout = 500, .exit = new Range(3, 20)});
    tsukasa.toggle();
...
tsukasa.toggle();
    ...
    //was 14.3
      chass.mtpoint( blueAS, {.settleRange = -1, .drift = 8, .within = 14.9});
    pose = chass.getPose();
// std::cout << "x: " << pose.pos.x << " y: " << pose.pos.y << " heading: " << pose.heading.heading() << std::endl;
    ...
    Pt mogo = {-40, -115};
    // chass.setLin(_lin);
        double ang = chass.turnTo(mogo, {.timeout = 650 , .exit = new Range(8, 20), .reverse = true});
        intake.move(0);
...
void corner4() {
    ...
    Pt ring3 = {-79.4, -112};
    chass.setTurn(_180);
    double ang = chass.turnTo(ring3, {.timeout = 700, .exit = new Range(5, 10)});
    chass.setTurn(_90);
    ...
    chass.turnTo(ring4, {.timeout = 450, .exit = new Range(2, 30)});
...
void hang() {
    ...
    chass.turn(neg(135), {.timeout = 600});
    chass.setTurn(_90);
    chass.driveAngle(-1900, neg(135), {.timeout = 1000, .vMax = 64, .slew = 4,});
    pros::delay(100);
