...
void rwallNeg() {
      ...
      pros::delay(100);
      chass.driveAngle(930, 90, {.timeout = 600, .exit = new Range(40, 10), .slew = 5});
      // pros::delay(300);
      ...
      chass.driveAngle(-700, 130, {.vMin = 40, .exit = new Range(150, 10)});
      Pt ring2 = {22.1, -27};
      ringAngle = chass.turnTo(ring2, {.exit = new Range(14, 10)});

      lb.next();
      chass.driveAngle(830, ringAngle, {.async = true, .vMin = 70});
      pros::delay(200);
      ...
      chass.waitUntilSettled();
      chass.setTurn(_5);
      double ang = chass.turnTo(stake, {.timeout = 470});
      chass.setTurn(_90);
      lb.next();
      intake.move(127);
      pros::delay(100);
      chass.driveAngle(250, ang+10, {.timeout = 200,});
      chass.driveAngle(-250, ang-10, {.timeout = 200, });
      chass.driveAngle(250, ang+10, {.timeout = 200, });
      lb.next();
      ...
      chass.driveAngle(-400, ang, {.timeout = 200, .vMin= 80});
      chass.mtpoint(corner1, {.async = true, .timeout = 1300,.vMin = 70, .exit = new Range(1, 10), .drift = 6});
      pros::delay(500);
...
// chass.driveAngle(990, ang, {.timeout = 630, .slew = 3});
//             intake.setJamProtection(false);
